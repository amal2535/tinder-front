...
function Cardtinder() {
  const { profile, DistancePreference, onlyShowDistance, ageRange } = useContext(SidebarContext);
  ...
  useEffect(() => {
    const LoadUsers = async ({ gender, likedUsers, superLikedUsers }) => {
      try {
        ...
        if (res && res.data.Profiles) {
          const filteredProfiles = res.data.Profiles.filter(profile => {
            const age = calculateAge(profile.birthday);

            // Only show profiles within the specified age range and distance
            const isWithinAgeRange = age >= ageRange[0] && age <= ageRange[1];
            const isWithinDistance = calculateDistance(Location, profile?.Location) <= DistancePreference;
    ...
    LoadUsers({ gender, likedUsers, superLikedUsers, onlyShowDistance });
  }, [gender, likedUsers, superLikedUsers, DistancePreference, ageRange]);
