...
const Navbar = () => {
    return(
        <div className="fixed top-0 mb-8 w-full z-40 border-b pink-glassmorphism flex justify-around">
            ...
            { items.map((item, key)=>(
                      <h1 className="text-white font-semibold text-xl -translate-y-1 hover:cursor-pointer transition delay-150 hover:text-purple-900 hover:-translate-y-[5px]" key={key}> {item} </h1>
                )
                  ) }
            </div>
            </div>
            ...
            <div className="lg:flex hidden items-center justify-around gap-10 pl-12">
                ...
                <button className="bg-white text-black rounded-2xl px-7 py-1 font-semibold transition delay-75 text-lg hover:bg-gray-200"> 
                    Log in
                </button>
            </div>
